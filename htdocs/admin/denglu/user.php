...
assign_template();
	...
	if($_REQUEST['op'] == 'unbind' && !empty($_REQUEST['mediaUserID']) && !empty($_SESSION['user_id'])){/////////////在线解绑
		...
		show_message($Dlang['denglu_unbind_success'],$Dlang['show_my_info'],'denglu.php?actt=dl_user');
	}
	...
	if($_REQUEST['op'] == 'save' && !empty($_SESSION['user_id'])){//////////保存设置
		...
		show_message($Dlang['setting_success'],$Dlang['show_my_info'],'denglu.php?actt=dl_user&op=set');
	}
	...
	if($_REQUEST['op'] == 'olbind_do_have' && !empty($_SESSION['mediaUserID'])){///////绑定已有用户提交页
		if(ec_login() && !isbind($_SESSION['mediaUserID'])){
			...
			if(!olbind($_SESSION['mediaUserID'])){
				show_message($Dlang['denglu_bind_failed'],$Dlang['show_my_info'],'denglu.php?actt=dl_user');
			}
			...
			show_message($Dlang['denglu_bind_success'],$Dlang['show_my_info'],'denglu.php?actt=dl_user');
		}
		show_message($Dlang['denglu_bind_failed'],$Dlang['show_my_info'],'denglu.php?actt=dl_user');
