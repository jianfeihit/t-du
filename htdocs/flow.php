...
if ($_REQUEST['step'] == 'add_to_cart'){
    ...
    if (!is_numeric($goods->number) || intval($goods->number) <= 0){ /* 检查：商品数量是否合法 */
        $result['error']   = 1;
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET'){
        ...
        if (($captcha & CAPTCHA_LOGIN) && 
        		(!($captcha & CAPTCHA_LOGIN_FAIL) || 
        				(($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0){
        ...
        if (!empty($_POST['act']) && $_POST['act'] == 'signin'){
            $captcha = intval($_CFG['captcha']);
            if (($captcha & CAPTCHA_LOGIN) && 
            		(!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) 
...
elseif ($_REQUEST['step'] == 'done')
{
   ...
   /* 判断是不是实体商品 */
    ...
    if (isset($_POST['goods_number']) && isset($_POST['rec_id'])){
    	$data = array("346"=>2);
    	echo $data;
        flow_update_cart($data);
    }
...
}elseif ($_REQUEST['step'] == 'cart_receiver'){
    $smarty->display('t_cart_receiver.dwt');
}elseif ($_REQUEST['step'] == 'cart_pay'){
...
function flow_update_cart($arr){
    ...
    foreach ($arr AS $key => $val){
        ...
        if (intval($GLOBALS['_CFG']['use_storage']) > 0 
        		&& $goods['extension_code'] != 'package_buy'){ //查询：系统启用了库存，检查输入的商品数量是否有效
        ...
        }elseif (intval($GLOBALS['_CFG']['use_storage']) > 0 
        		&& $goods['extension_code'] == 'package_buy'){
