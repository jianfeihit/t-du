...
if ($_REQUEST ['step'] == 'add_to_cart') {
	...
	if (! is_numeric ( $goods->number ) || intval ( $goods->number ) <= 0) { /*
		...
		if (addto_cart ( $goods->goods_id, $goods->number, $goods->spec, $goods->parent )) { // 更新：添加到购物车
...
} elseif ($_REQUEST ['step'] == 'cart_deliver') {//收货人信息,对应原来的 consignee
	...
	if ($_SERVER ['REQUEST_METHOD'] == 'GET') {
		...
		if ($_SESSION ['user_id'] > 0) { //获得用户所有的收货人信息
		...
		$smarty->display ( 't_cart_deliver.dwt' );
	} else { //保存收货人信息
		$consignee = array (
				...
				'address' => empty ( $_POST ['address'] ) ? '' : $_POST ['address'],//OK
...
// 		exit ();
...
} elseif ($_REQUEST ['step'] == 'drop_consignee') {//删除收货人信息
	include_once ('includes/lib_transaction.php');
...
} elseif ($_REQUEST ['step'] == 'cart_confirm') { // 订单确认。 对应原来的checkout
    /* 取得购物类型 */
    $flow_type = isset ( $_SESSION ['flow_type'] ) ? intval ( $_SESSION ['flow_type'] ) : CART_GENERAL_GOODS;
	 ...
	 */
	if (empty ( $_SESSION ['direct_shopping'] ) && $_SESSION ['user_id'] == 0) { //用户没有登录且没有选定匿名购物，转向到登录页面
		ecs_header ( "Location: flow.php?step=login\n" );
		exit ();
	}
	
	$consignee = get_consignee ( $_SESSION ['user_id'] );
// 	echo print_r(check_consignee_info ( $consignee, $flow_type ),true);
	
	/* 检查收货人信息是否完整 */
	if (! check_consignee_info ( $consignee, $flow_type )) {
		/* 如果不完整则转向到收货人信息填写界面 */
		ecs_header ( "Location: flow.php?step=cart_deliver\n" );
		exit ();
	}
	...
	$smarty->display ( 't_cart_confirm.dwt' );
} elseif ($_REQUEST ['step'] == 'cart_receiver') { // 改变配送方式
	include_once ('includes/cls_json.php');
	$json = new JSON ();
	$result = array (
			'error' => '',
	...
	echo $json->encode ( $result );
	$smarty->display ( 't_cart_receiver.dwt' );
	exit ();
	...
	echo $json->encode ( $result );
	exit ();
} elseif ($_REQUEST ['step'] == 'cart_pay') { // 改变支付方式
	include_once ('includes/cls_json.php');
	$json = new JSON ();
	$result = array (
			'error' => '',
...
elseif ($_REQUEST ['step'] == 'done') {
...
} elseif ($_REQUEST ['step'] == 'update_cart') { // 更新购物车
	if (isset ( $_GET ['goods_number'] ) && isset ( $_GET ['rec_id'] )) {
		flow_update_cart ( $_GET ['rec_id'], $_GET ['goods_number'] );
	}
	echo 'OK';
	exit ();
} elseif ($_REQUEST ['step'] == 'drop_goods') { // 删除购物车中的商品
	$rec_id = intval ( $_GET ['id'] );
	flow_drop_cart_goods ( $rec_id );
	ecs_header ( "Location: flow.php\n" );
	exit ();
} elseif ($_REQUEST ['step'] == 'add_favourable') { /* 取得优惠活动信息 */
	...
	ecs_header ( "Location: flow.php\n" );
	exit ();
} elseif ($_REQUEST ['step'] == 'validate_bonus') { // 验证红包序列号
	...
	$json = new JSON ();
	die ( $json->encode ( $result ) );
} elseif ($_REQUEST ['step'] == 'add_package_to_cart') { // 添加礼包到购物车
	include_once ('includes/cls_json.php');
	...
	$result ['confirm_type'] = ! empty ( $_CFG ['cart_confirm'] ) ? $_CFG ['cart_confirm'] : 2;
	die ( $json->encode ( $result ) );
} elseif ($_REQUEST ['step'] == 'cart_detail') {
	/* 标记购物流程为普通商品 */
 ...
 * @return void
 */
function flow_update_cart($key, $val) {
	/* 处理 */
	...
	if (intval ( $GLOBALS ['_CFG'] ['use_storage'] ) > 0 && $goods ['extension_code'] != 'package_buy') { // 查询：系统启用了库存，检查输入的商品数量是否有效
