...
if (!$smarty->is_cached('t_goodsVote.dwt', $cache_id))
{
    /* 如果页面没有被缓存则重新获取页面的内容 */
...
$smarty->display('t_goodsVote.dwt', $cache_id);
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot,g.toupiao,g.cat_id, g.shop_price AS org_price, case when gvote.vote_count is null then 0 else gvote.vote_count end as votes,' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
			'LEFT JOIN ' . $GLOBALS['ecs']->table('good_vote') . ' AS gvote ' .
                "ON gvote.good_id = g.goods_id " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['name']             = $row['goods_name'];
		 $arr[$row['goods_id']]['toupiao']             =  $row['toupiao'];
		...
		$arr[$row['goods_id']]['votes']             = $row['votes'];
    }
