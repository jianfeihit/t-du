...
if (!$smarty->is_cached('t_category.dwt', $cache_id))
{
    /* 如果页面没有被缓存则重新获取页面的内容 */
...
$smarty->display('t_category.dwt', $cache_id);
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot,g.toupiao,g.cat_id, g.shop_price AS org_price, ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['promote_end_date']        = local_date($GLOBALS['_CFG']['date_format'], $row['promote_end_date'] );
    }
